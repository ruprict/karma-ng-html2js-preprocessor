...
var createHtml2JsPreprocessor = function(logger, basePath, config) {
  ...
  var removeFromPath = config.removeFromPath || []; 
  var stripPrefix = new RegExp('^' + (config.stripPrefix || ''));
  var prependPrefix = config.prependPrefix || '';
  var cacheIdFromPath = config && config.cacheIdFromPath || function(filepath) {
    var fName = filepath;
    removeFromPath.forEach(function(strToRemove) {
      fName = fName.replace(strToRemove,''); 
    });
    return prependPrefix + fName;//filepath.replace(stripPrefix, '');
  };
